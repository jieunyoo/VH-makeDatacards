...
def smass(sName):
    if sName in ['ggF','VBF','WH','ZH','ttH']:
        _mass = 125.
    elif sName in ['WJetsLNu','EWKW','TTbar','SingleTop','VV']:
        _mass = 80.379
...
def dummy_rhalphabet( scale_syst=True, smear_syst=True,  systs=True,muonCR=False,year='2017',  opts=None):
    ...
    for region in ['pass', 'fail']:
        ...
        include_samples = [ # to do: add other samples
                'WJetsLNu', 'DYJets', 'TTbar', 'SingleTop',
            ]
    ...
    #***Morphing of templates here, do for all samples (except data, Fakes)
            ...
            def smorph(templ,sName):      
                ...
                if MORPHNOMINAL and sName not in ['QCD']:
                    return MorphHistW2(templ).get(shift=SF[year]['shift_SF']/smass('WJetsLNu') * smass(sName), smear=SF[year]['smear_SF'] )
                else:
                    return templ
 ...
 #for region in ['pass', 'fail']:
    ...
    with open('testModel_top.root', 'wb') as fout:
    #with open("{}.pkl".format(model_name), "wb") as fout:
        pickle.dump(model, fout)

    model.renderCombine('testModel_top')

  

#******************************************************
ROOTFILE = '/home/jieun/oct10/2017/2017_TopCR.root'
#******************************************************
